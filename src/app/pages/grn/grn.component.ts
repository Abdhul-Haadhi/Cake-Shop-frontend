...
export class GrnComponent implements OnInit {
  ...
  constructor(
    ...
    private demoService: GRNServiceService,
    ...
    try {
      if (this.mode === 'edit') {
        ...
        if (checked == 1 || this.isinnerEdit == true) {
          this.demoService
            .editDataOuterForm(this.selectedData?.grnno, this.demoForm.value)
            .subscribe({
              next: (response: any) => {
        ...
        this.demoService
          .serviceCallPost(this.demoForm.value)
          .subscribe((response) => {
  ...
  onSubmitInner() {
    try {
      if (this.innermode === 'inneredit') {
        ...
        this.demoService
          .innerEditData(this.innerselectedData?.id, this.innerForm.value)
          .subscribe({
            next: (response: any) => {
              ...
              setTimeout(() => {
                ...
                if (this.mode == 'edit') {
                  ...
                  this.demoService
                    .stockUpdateEdit(innerItem)
                    .subscribe((response) => {
                      console.log(
                        'post data Server Stock update edit Response',
                        response
                      );
                    });
                }
